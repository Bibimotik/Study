...
public partial class ShowServiceSheet : Window
{
    public int customer_id;
    
    public ShowServiceSheet()
    {
        InitializeComponent();
    }
        ...
        if (IsPositiveNumber(phone_text.Text) && IsWithinMaxLength(phone_text.Text, 16))
        {
            using (NpgsqlConnection connection = new NpgsqlConnection(SQLclass.user_string))
            {
                ...
                using (NpgsqlTransaction transaction = connection.BeginTransaction())
                {
                    try
                    {
                        using (NpgsqlCommand command = new NpgsqlCommand("SELECT * FROM get_servicesheet(@cust_id)", connection))
                        {
                            ...
                            using (NpgsqlDataReader reader = command.ExecuteReader())
                            {
                                ...
                                table.Load(reader);
                                serviceSheet.ItemsSource = table.DefaultView;
                            }
                    ...
                    catch (Exception ex)
                    {
                        ...
                        MessageBox.Show($"Ошибка при получении записей на обслуживание: {ex.Message}");
                    }
                }
            }
        }
