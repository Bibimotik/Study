...
public partial class SparePartsBalances : Window
{
    public SparePartsBalances()
    {
        InitializeComponent();
        ...
        using (NpgsqlConnection connection = new NpgsqlConnection(SQLclass.manager_string))
        {
            ...
            using (NpgsqlTransaction transaction = connection.BeginTransaction())
            {
                try
                {
                    ...
                    using (NpgsqlCommand countCommand = new NpgsqlCommand("SELECT COUNT(*) FROM get_available_spareparts()", connection))
                    {
